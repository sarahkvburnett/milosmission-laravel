    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($row->type == 'relationship' && $row->details->type == 'hasMany'){
            ...
            foreach ($content as $id){
                $models[] = $row->details->model::find($id);
            }
            $data->hasMany($row->details->model)->saveMany($models);
            $row->details->model::where($row->details->column, $data->id)->whereNotIn('id', $content)->update([$row->details->column => NULL]);
        }
